...
const sponsorChannels = []; // [{ username, label }]
...
function sess(id) {
  if (!sessions.has(id)) sessions.set(id, { step: "idle", partnerId: null, disconnectPartnerId: null });
...
const KB = {
  main:    kb([
    ...
    ["👥 پیام ناشناس به گروه", "🔗 لینک ناشناس من"],
    ["🏆 افزایش امتیاز", "راهنما"],
  ]),
  ...
  blockReason: kb([
    ["جنسیتش اشتباه بود", "بی ادب بود"],
    ["باهاش حال نکردم", "تبلیغ فرستاد"],
    ["بیخیال، بعداً هم وصل شم"],
  ], true),
...
async function getMissing(userId) {
  ...
  for (const ch of sponsorChannels) {
    try {
      ...
      if (!["member", "administrator", "creator"].includes(m.status)) out.push(ch);
    } catch { out.push(ch); }
  }
  return out;
}
...
bot.on("callback_query", async q => {
  ...
  if (data === "check_join") {
    ...
    if (!miss.length) {
      bot.answerCallbackQuery(q.id, { text: "✅ تأیید شد!" });
...
bot.onText(/\/banner/, async (msg) => {
  ...
  send(userId,
    `یا این برنامه میتونی هر وقت بخوای به صورت تصادفی به یک نفر وصل بشی و کاملاً ناشناس گپ بزنی\n\n👇 شروع کن\n${refLink}`,
    KB.main
  );
});
...
bot.onText(/\/ban (\d+)/, (msg, match) => {
  ...
  if (u) { u.banned = true; doDisconnect(tId, true); send(msg.chat.id, `✅ کاربر ${tId} بن شد.`); }
...
bot.on("message", async (msg) => {
  ...
  if (s.step === "admin_add_sponsor") {
    ...
    s.step = "idle";
    send(userId, `✅ اسپانسر اضافه شد!\n📌 ${ch}\n🏷 ${label}\nتعداد: ${sponsorChannels.length}`, KB.admin);
    return;
  }
  ...
  if (isAdmin(msg)) {
    ...
    if (text === "➕ اضافه اسپانسر") { s.step = "admin_add_sponsor"; send(userId, "📌 یوزرنیم کانال رو بفرست (مثال: @mychannel):", KB.cancel); return; }
    if (text === "🗑 حذف اسپانسر") {
      ...
      bot.sendMessage(userId, "کدوم اسپانسر حذف بشه؟", { reply_markup: { inline_keyboard: inline } });
      return;
    }
  ...
  if (s.step === "waiting") {
    ...
    else send(userId, "⏳ هنوز داری منتظری...\n\nاگه تا حداکثر یک دقیقه پیامی نرسید دوباره تلاش کن.", KB.waiting);
    return;
  }
  ...
  if (s.step === "in_chat") {
    if (text === "قطع مکالمه") {
      s.disconnectPartnerId = s.partnerId;
      s.step = "confirm_disconnect";
